...
if __name__ == "__main__":
    ...
    for dirname in os.listdir(root):
        if os.path.isdir(os.path.join(root, dirname)):
            for dirname2 in os.listdir(os.path.join(root, dirname)):
                latest_run_dir = f"{root}/{dirname}/{dirname2}/wandb/latest-run"
                if is_file_updated_inwandb(latest_run_dir):
                    command_list.append(
                        f"wandb sync {latest_run_dir} --append"
                    )  # https://github.com/wandb/wandb/issues/2423#issuecomment-1528643531
