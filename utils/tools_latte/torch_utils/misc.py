...
def assert_shape(tensor, ref_shape):
    ...
    if tensor.ndim != len(ref_shape):
        raise AssertionError(
            f"Wrong number of dimensions: got {tensor.ndim}, expected {len(ref_shape)}{err_suffix}"
        )
        ...
        if ref_size is None:
            ...
            with suppress_tracer_warnings():  # as_tensor results are registered as constants
                symbolic_assert(
                    ...
                    f"Wrong size for dimension {idx}{err_suffix}",
                )
        elif isinstance(size, torch.Tensor):
            with suppress_tracer_warnings():  # as_tensor results are registered as constants
                symbolic_assert(
                    torch.equal(size, torch.as_tensor(ref_size)),
                    f"Wrong size for dimension {idx}: expected {ref_size}{err_suffix}",
                )
        elif size != ref_size:
            raise AssertionError(
                f"Wrong size for dimension {idx}: got {size}, expected {ref_size}{err_suffix}"
            )
...
def check_ddp_consistency(module, ignore_regex=None):
    ...
    for name, tensor in named_params_and_buffers(module):
        ...
        assert (
            nan_to_num(tensor) == nan_to_num(other)
        ).all(), f"{fullname} is not DDP consistent"
...
def print_module_summary(module, inputs, max_nesting=3, skip_redundant=True):
    ...
    def post_hook(mod, module_inputs, outputs):
        ...
        if nesting[0] <= max_nesting:
            ...
            entries.append(
                dnnlib.EasyDict(mod=mod, inputs=module_inputs, outputs=outputs)
            )
    ...
    ), f"Summary table contains rows of different lengths: {row_lengths}"
