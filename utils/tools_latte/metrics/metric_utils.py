...
from utils.tools_latte.utils.dataset import video_to_image_dataset_kwargs
...
class ProgressMonitor:
    ...
    def update(self, cur_items: int):
        assert (self.num_items is None) or (
            cur_items <= self.num_items
        ), f"Wrong `items` values: cur_items={cur_items}, self.num_items={self.num_items}"
...
def compute_feature_stats_for_dataset(
    ...
    max_items=None,
    temporal_detector=False,
    use_image_dataset=False,
    feature_stats_cls=FeatureStats,
    **stats_kwargs,
):

    dataset_kwargs = (
        video_to_image_dataset_kwargs(opts.dataset_kwargs)
        if use_image_dataset
        else opts.dataset_kwargs
    )
    ...
    item_subset = random.sample(
        range(data_length), num_items
    )  # added by xin, randomly selected 2048 videos
...
def compute_feature_stats_for_generator(
    ...
    **stats_kwargs,
    ...
    def run_generator(z, c, t):
        ...
        if temporal_detector:
            img = img.view(
                bt // num_video_frames, num_video_frames, c, h, w
            )  # [batch_size, t, c, h, w]
    ...
    if jit:
        ...
        t = torch.zeros(
            [batch_gen, G.cfg.sampling.num_frames_per_video], device=opts.device
        )
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            cond_sample_idx = [
                np.random.randint(len(dataset)) for _ in range(batch_gen)
            ]
            c = [dataset.get_label(i) for i in cond_sample_idx]
            c = torch.from_numpy(np.stack(c)).pin_memory().to(opts.device)
            t = [
                list(range(0, num_video_frames * subsample_factor, subsample_factor))
