...
from utils.tools_latte.utils.layers import sample_frames
...
class Dataset(torch.utils.data.Dataset):
    ...
    def label_dim(self):
        assert (
            len(self.label_shape) == 1
        ), f"Labels must be 1-dimensional: {self.label_shape} to use `.label_dim`"
...
class ImageFolderDataset(Dataset):
    ...
    def _load_raw_labels(self):
        ...
        labels = dict(labels)
        labels = [
            labels[remove_root(fname, self._name).replace("\\", "/")]
...
class VideoFramesFolderDataset(Dataset):
    def __init__(
        ...
        **super_kwargs,  # Additional arguments for the Dataset base class.
        ...
        self.sampling_dict = (
            OmegaConf.to_container(OmegaConf.create({**cfg.sampling}))
            if "sampling" in cfg
            else None
        )
        ...
        if self.subsample_factor > 1 and self.load_n_consecutive is None:
            raise NotImplementedError(
                "Can do subsampling only when loading consecutive frames."
            )

        listdir_full_paths = lambda d: sorted(
        ...
        if os.path.isdir(self._path):
            ...
            self._all_objects = {
                o
                for d in listdir_full_paths(self._path)
                for o in (([d] + listdir_full_paths(d)) if os.path.isdir(d) else [d])
            }
            self._all_objects = {
                os.path.relpath(o, start=os.path.dirname(self._path))
                for o in {self._path}.union(self._all_objects)
            }
        elif self._file_ext(self._path) == ".zip":
            self._type = "zip"
        ...
        for o in objects[1:]:
            ...
            if self._file_ext(o) in PIL.Image.EXTENSION:
                assert o.startswith(
                    curr_d
                ), f"Object {o} is out of sync. It should lie inside {curr_d}"
                assert (
                    curr_obj_depth == root_path_depth + 2
                ), "Frame images should be inside directories"
                ...
                assert (
                    curr_obj_depth == root_path_depth + 1
                ), "Classes info file should be inside the root dir"
                pass
            else:
                # We encountered a new directory
                assert (
                    curr_obj_depth == root_path_depth + 1
                ), f"Video directories should be inside the root dir. {o} is not."
        ...
        if self.discard_short_videos:
            self._video_dir2frames = {
                d: fs
                for d, fs in self._video_dir2frames.items()
                if len(fs) >= self.load_n_consecutive * self.subsample_factor
            }
        ...
        raw_shape = [len(self._video_idx2frames)] + list(
            self._load_raw_frames(0, [0])[0][0].shape
        )
    ...
    def _load_raw_labels(self):
        ...
        labels = video_labels
        labels = [
            labels[os.path.normpath(dname).split(os.path.sep)[-1]]
            for dname in self._video_dir2frames
        ]
        labels = np.array(labels)
    ...
    def __getitem__(self, idx: int) -> Dict:
        if self.load_n_consecutive:
            ...
            assert (
                num_frames_available - self.load_n_consecutive * self.subsample_factor
                >= 0
            ), f"We have only {num_frames_available} frames available, cannot load {self.load_n_consecutive} frames."

            if self.load_n_consecutive_random_offset:
                random_offset = random.randint(
                    0,
                    num_frames_available
                    - self.load_n_consecutive * self.subsample_factor
    ...
    def _load_raw_frames(
        ...
        if frames_idx is None:
            assert (
                not self.sampling_dict is None
            ), f"The dataset was created without `cfg.sampling` config and cannot sample frames on its own."
            if total_len > self.max_num_frames:
                offset = random.randint(0, total_len - self.max_num_frames)
            frames_idx = (
                sample_frames(
                    self.sampling_dict,
                    total_video_len=min(total_len, self.max_num_frames),
                )
                + offset
            )
        else:
...
def video_to_image_dataset_kwargs(
    video_dataset_kwargs: dnnlib.EasyDict,
) -> dnnlib.EasyDict:
