...
from utils.tools_latte.torch_utils import persistence
from utils.tools_latte.torch_utils.ops import bias_act, upfirdn2d, conv2d_resample
from utils.tools_latte.torch_utils import misc
...
class FullyConnectedLayer(torch.nn.Module):
    def __init__(
        ...
        self.bias = (
            torch.nn.Parameter(torch.full([out_features], float(bias_init)))
            if bias
            else None
        )
...
class Conv2dLayer(torch.nn.Module):
    ...
    def forward(self, x, gain=1):
        ...
        b = (
            (self.bias.to(x.dtype) * self.lr_multiplier)
...
class GenInput(nn.Module):
    ...
    def forward(
        self,
        batch_size: int,
        motion_v: Optional[torch.Tensor] = None,
        dtype=None,
        memory_format=None,
    ) -> torch.Tensor:
        if self.cfg.input.type == "const":
    ...
    def get_dim(self) -> int:
        if self.cfg.sampling.num_frames_per_video == 1:
            ...
            if self.cfg.sampling.type == "uniform":
                ...
                return (self.d + self.time_encoder.get_dim()) * (
                    self.cfg.sampling.num_frames_per_video - 1
                )
    ...
    def forward(self, t: torch.Tensor) -> torch.Tensor:
        ...
        if self.cfg.sampling.num_frames_per_video == 1:
            ...
            if self.cfg.sampling.type == "uniform":
                ...
                t_diffs = (t[:, 1:] - t[:, :-1]).view(
                    -1
                )  # [batch_size * (num_frames - 1)]
            # Note: float => round => long is necessary when it's originally long
            const_embs = self.const_embed(
                t_diffs.float().round().long()
            )  # [batch_size * num_diffs_to_use, d]
            fourier_embs = self.time_encoder(
                t_diffs.unsqueeze(1)
            )  # [batch_size * num_diffs_to_use, num_fourier_feats]
            out = torch.cat(
                [const_embs, fourier_embs], dim=1
...
class FixedTimeEncoder(nn.Module):
    def __init__(
        ...
        fourier_coefs = construct_log_spaced_freqs(
            max_num_frames, skip_small_t_freqs=skip_small_t_freqs
        )
    ...
    def forward(self, t: torch.Tensor) -> torch.Tensor:
        ...
        fourier_raw_embs = self.fourier_coefs * t.unsqueeze(
            1
        )  # [bf, num_fourier_feats]
...
class EqLRConv1d(nn.Module):
    def __init__(
        ...
        self.activation = activation
        self.weight = torch.nn.Parameter(
            torch.randn([out_features, in_features, kernel_size]) / lr_multiplier
        )
        self.bias = (
            torch.nn.Parameter(torch.full([out_features], float(bias_init)))
            if bias
...
def random_frame_sampling(
    ...
    if type(cfg.get("total_dists")) in (list, tuple):
        time_diff_range = [
            d for d in cfg["total_dists"] if min_time_diff <= d <= max_time_diff
        ]
    else:
        ...
        frames_idx.extend(
            [
                ...
                for t in random.sample(
                    range(1, time_diff), k=cfg["num_frames_per_video"] - 2
                )
            ]
        )
...
def uniform_frame_sampling(
    ...
    if type(cfg.get("dists_between_frames")) in (list, tuple):
        valid_dists = [
            d for d in cfg["dists_between_frames"] if d <= ["max_dist_between_frames"]
        ]
        valid_dists = [
            d
            for d in valid_dists
            if (d * cfg["num_frames_per_video"] - d + 1) <= total_video_len
        ]
        d = random.choice(valid_dists)
    else:
        max_dist = min(
            cfg.get("max_dist", float("inf")),
            total_video_len // cfg["num_frames_per_video"],
        )
...
def construct_log_spaced_freqs(
    ...
    powers = (
        torch.tensor([2]).repeat(num_fourier_feats).pow(torch.arange(num_fourier_feats))
    )  # [num_fourier_feats]
