...
class ClipCaptionModel(nn.Module):
    ...
    def __init__(self, prefix_length: int, hidden_dim=None):
        ...
        self.gpt = GPT2LMHeadModel.from_pretrained(
            "gpt2", eos_token_id=base_tokenizer.eos_token_id
        )
        self.gpt.resize_token_embeddings(len(base_tokenizer))

        self.hidden_dim = hidden_dim
        self.encode_prefix = (
            nn.Linear(768, hidden_dim) if hidden_dim is not None else nn.Identity()
        )
        self.decode_prefix = (
            nn.Linear(hidden_dim, 768) if hidden_dim is not None else nn.Identity()
        )
...
class CaptionDecoder(object):
    def __init__(self, device, pretrained_path, hidden_dim=-1):
        ...
        assert len(mk) == 0
        # assert all([name.startswith("clip") for name in uk])# commented by Tao Hu
    ...
    def generate_captions(
        ...
        with torch.no_grad():
            for feature in features:
                feature = self.caption_model.decode_prefix(
                    feature.to(self.device)
                )  # back to the clip feature
                if use_beam_search:
                    generated_captions.append(
                        generate_beam(
                            self.caption_model, self.tokenizer, embed=feature
                        )[0]
                    )
                else:
                    generated_captions.append(
                        generate2(self.caption_model, self.tokenizer, embed=feature)
                    )
...
if __name__ == "__main__":
    decoder = CaptionDecoder(
        device="cuda", pretrained_path="pretrained_ckpt/caption_decoder.pth"
    )
    features = torch.randn(1, 77, 768)
    print(decoder.generate_captions(features))
