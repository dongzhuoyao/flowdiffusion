...
class FrechetVideoDistance(Metric):
    ...
    def update(self, videos: Tensor, real: bool) -> None:
        ...
        videos = videos.permute(0, 4, 1, 2, 3)  # [B,C,T,W,H]
        ...
        if real:
            ...
            self.real_features_num_samples += len(videos)
        else:
            self.fake_features_sum += features.sum(dim=0)
            self.fake_features_cov_sum += features.t().mm(features)
            self.fake_features_num_samples += len(videos)

    def compute(self) -> Tensor:
        """Calculate FID score based on accumulated extracted features from the two distributions."""
        if self.real_features_num_samples < 2 or self.fake_features_num_samples < 2:
            raise RuntimeError(
                f"More than one sample is required for both the real and fake distributed to compute FID,self.real_features_num_samples={self.real_features_num_samples},self.fake_features_num_samples={self.fake_features_num_samples}"
            )
