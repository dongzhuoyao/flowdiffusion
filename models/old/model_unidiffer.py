...
class UViT(nn.Module):
    ...
    def forward(self, img, clip_img, text, t_img, t_text, data_type):
        ...
        x = torch.cat(
            (t_img_token, t_text_token, token_embed, text, clip_img, img), dim=1
        )
        ...
        if H == self.img_size[0] and W == self.img_size[1]:
            ...
            pos_embed_others, pos_embed_patches = torch.split(
                pos_embed, [1 + 1 + 1 + num_text_tokens + 1, self.num_patches], dim=1
            )
        ...
        (
            ...
            clip_img_out,
            img_out,
        ) = x.split((1, 1, 1, num_text_tokens, 1, num_img_tokens), dim=1)
