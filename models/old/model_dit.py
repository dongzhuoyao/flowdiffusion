...
class FeatEmbedder(nn.Module):
    ...
    Embeds class labels into vector representations. Also handles label dropout for classifier-free guidance.
    """
    def __init__(self, hyper_dim=128, hidden_size=None):
        super().__init__()
        self.cfg_embedding = nn.Embedding(1, hyper_dim)
        self.mlp = nn.Sequential(
            nn.Linear(hyper_dim, hidden_size, bias=True),
            nn.SiLU(),
            nn.Linear(hidden_size, hidden_size, bias=True),
        )

    def forward(self, feats, train, cond_drop_prob):
        # feats shape: [B, C]
        bs = len(feats)
        use_dropout = cond_drop_prob > 0
        if use_dropout:
            drop_ids = torch.rand(bs, device=feats.device) < cond_drop_prob

            _cfg_embedding = repeat(self.cfg_embedding.weight, "1 c -> b c", b=bs)
            feats = torch.where(drop_ids.unsqueeze(1), _cfg_embedding, feats)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        print(f"use_shortcut: {use_shortcut}")

        self.x_embedder = PatchEmbed(
            img_dim, patch_size, in_channels, embed_dim, bias=True
        )
        ...
        if num_classes > 0:
            if cond_type == "label":
                ...
                raise ValueError(f"cond_type {cond_type} not supported")
    ...
    def initialize_weights(self):
        ...
        if self.num_classes > 0:
            ...
            elif self.cond_type == "feat":
                nn.init.normal_(self.y_embedder.cfg_embedding.weight, std=0.02)
            else:
                raise ValueError(f"cond_type {self.cond_type} not supported")
    ...
    def forward(self, x, t, y=None, cond_drop_prob=0.1):
        """
        Forward pass of DiT.
    ...
    def forward_without_cfg(self, x, t, y=None):
        return self.forward(x, t, y, cond_drop_prob=0.0)

    def forward_with_cfg(self, x, t, y, cfg_scale):
        uncond = self.forward(x, t, y, cond_drop_prob=1.0)
        if cfg_scale == 0.0:
            return uncond
        cond = self.forward(x, t, y, cond_drop_prob=0.0)
        return uncond + cfg_scale * (cond - uncond)

    def forward_with_cfg_old(self, x, t, y, cfg_scale):
        """
...
if __name__ == "__main__":
    ...
    model = DiT_B_2(in_channels=4, num_classes=-1, learn_sigma=False, img_dim=32)
